...
#             print len(image), image[0].shape, image[1].shape, image[2].shape, image[3].shape
        ...
        images = np.stack((images[0][::1], images[0][::-1]) + tuple([np.stack(tuple([images[index][random_index] for random_index in order]), axis=0) for order in self.order_type]), axis=0)
        
#         print "After labelling"
#         for image in images:
#             print len(image[1]), image[1][0].shape, image[1][1].shape, image[1][2].shape, image[1][3].shape

        labels = np.array([1. if index_label <=1 else 0. for index_label in xrange(24)], dtype=np.float32)
...
# PICKLE_PATH = os.path.join(HOME_DIR, 'patch.pkl')
# cds = ChronoDataSet(PICKLE_PATH, HDF5_PATH)
# print cds.__getitem__(50)[0].shape, cds.__getitem__(50)[1].shape
# print cds.__getitem__(50)[1]
