...
#             print len(image), image[0].shape, image[1].shape, image[2].shape, image[3].shape
        rnd = [self.order_type[np.random.randint(0, len(self.order_type))] for index in xrange(self.batch_size)] 
    
        # Randomly +ve and -ve test case
        images = [(np.stack(tuple([images[index][random_index] 
                  for random_index in rnd[index]]), axis=0), 0) 
                  if True or np.random.randint(0,2) else (images[index][::1 * ((-1)**(np.random.randint(0,2) + 1))], 1) for index in xrange(self.batch_size)]
